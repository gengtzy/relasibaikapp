...
<div x-data="{ 
            ...
            <div class="p-4 flex justify-end">
                <div class="relative w-full sm:w-64">
                    <div class="absolute inset-y-0 left-0 flex items-center ps-3 pointer-events-none">
                        <i class="fas fa-search text-gray-500 dark:text-slate-400"></i>
                    </div>
                    <input wire:model.live.debounce.300ms="search" type="text"
                        class="block w-full p-2.5 pl-10 text-sm text-slate-900 border border-slate-300 rounded-xl bg-gray-50 focus:ring-blue-500 focus:border-blue-500 shadow-sm dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:placeholder:text-slate-400 transition-all"
                        placeholder="Cari lokasi atau tanggal...">
                </div>
            </div>

            {{-- Tabel Data --}}
            <div class="bg-white dark:bg-slate-800 overflow-hidden rounded-b-2xl">
                @if ($screenings->count() > 0)
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-slate-500 dark:text-slate-300">
                            <thead class="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200 dark:text-slate-200 dark:bg-slate-700 dark:border-slate-600">
                                <tr>
                            ...
                            <tbody>
                                @foreach ($screenings as $index => $item)
                                    <tr onclick="window.location='{{ route('screening.result', $item->id) }}'"
                                        class="bg-white border-b hover:bg-blue-50/50 transition-colors group cursor-pointer dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700/50">
                                        
                                        <td class="px-6 py-4 text-center">
                                            {{ $screenings->firstItem() + $index }}
                                        </td>
                                        <td class="px-6 py-4 font-medium text-slate-900 whitespace-nowrap dark:text-slate-100">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center">
                                                <svg class="w-3.5 h-3.5 text-slate-400 mr-2 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <td class="px-6 py-4">
                                            @if ($item->recommendation)
                                                <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full border border-blue-200 dark:bg-blue-900/30 dark:text-blue-200 dark:border-blue-800">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            <div class="flex justify-center items-center gap-3" @click.stop>
                                                ...
                                                <a href="{{ route('screening.result', ['resultId' => $item->id, 'action' => 'print']) }}"
                                                    target="_blank"
                                                    class="text-slate-400 hover:text-blue-500 transition-colors p-2 hover:bg-blue-50 rounded-full dark:hover:bg-slate-600 dark:hover:text-blue-400"
                                                    title="Cetak PDF">
                                                ...
                                                <button type="button"
                                                    ...
                                                    "
                                                    class="text-slate-400 hover:text-red-500 transition-colors p-2 hover:bg-red-50 rounded-full dark:hover:bg-slate-600 dark:hover:text-red-400"
                                                    title="Hapus">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @else
                    ...
                    <div class="flex flex-col items-center justify-center py-16 text-center">
                        <div class="p-4 rounded-full bg-slate-50 mb-4 dark:bg-slate-700/50">
                        ...
                        <h3 class="text-lg font-medium text-slate-900 dark:text-slate-200">Belum ada riwayat</h3>
                        <p class="text-slate-500 mt-2 max-w-sm dark:text-slate-400">
                            @if (!empty($search))
    ...
    <div x-show="showModal" style="display: none;" 
        ...
        <div @click.outside="showModal = false" 
            ...
            class="relative w-full max-w-sm bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 text-center border border-slate-100 dark:border-slate-700">

            {{-- Icon Visual --}}
            <div class="w-16 h-16 bg-red-100 dark:bg-red-900/30 text-red-500 dark:text-red-400 rounded-full flex items-center justify-center mx-auto mb-6">
            ...
            <div class="flex flex-col gap-3">
                ...
                <button @click="showModal = false"
                    class="w-full py-3 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 font-semibold rounded-lg transition-colors">
                    Batal
                </button>
            </div>
        </div>
