...
@close-modal.window="showModal = false">
    ...
    <div class="flex items-start justify-between">
        <h1 class="text-2xl font-bold mb-6">Pertanyaan</h1>

        <button wire:click="createNewQuestion" type="button"
    ...
    <div class="border border-slate-300 shadow-sm rounded-2xl bg-white">
            ...
            @if (count($selectedQuestions) > 0)
                <div class="w-full md:w-auto">
                    <button 
                        @click="
                            isBulk = true;
                            bulkCount = {{ count($selectedQuestions) }};
                            showModal = true;
                        ...
                        Hapus Pilihan ({{ count($selectedQuestions) }})
                    </button>
                </div>
            @else
                <div></div>
            ...
            <div class="flex space-x-4">
                <div class="relative">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" id="table-search"
                        ...
                        placeholder="Cari pertanyaan, instrumen, atau tipe">
                </div>
                <button type="button"
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Tipe Skoring</th>
                ...
                <tbody>
                    @forelse($questions as $question)
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <input wire:model.live="selectedQuestions" type="checkbox"
                                        value="{{ $question->id }}"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm accent-blue-500">
                                    <label class="sr-only">checkbox</label>
                                </div>
                            ...
                            <td wire:click="editQuestion({{ $question->id }})" class="px-6 py-4 cursor-pointer">
                                {{ $question->instrument->code ?? 'N/A' }}
                            </td>
                            <td wire:click="editQuestion({{ $question->id }})" class="px-6 py-4 cursor-pointer">
                                {{ Str::limit($question->question_text, 60) }} {{-- Limit text agar tabel rapi --}}
                            </td>
                            <td wire:click="editQuestion({{ $question->id }})" class="px-6 py-4 cursor-pointer">
                            ...
                            <td class="flex gap-2 px-6 py-4">
                                ...
                                <button type="button"
                                    @click="
                                        ...
                                        deleteCode = '{{ Str::limit($question->question_text, 30) }}'; {{-- Kirim potongan teks --}}
                                        $wire.set('deleteId', {{ $question->id }}, false);
        ...
        <nav class="flex pb-4 items-center flex-col md:flex-row justify-between pt-4 px-4"
            ...
            <span class="text-sm font-normal text-slate-500 mb-4 md:mb-0 block w-full md:inline md:w-auto">
                Showing
                <span class="font-semibold text-slate-700">{{ $questions->firstItem() ?? 0 }}-{{ $questions->lastItem() ?? 0 }}</span>
                of
                <span class="font-semibold text-slate-700">{{ $questions->total() }}</span>
            </span>
            {{ $questions->links() }}
        </nav>
                ...
                <div class="relative bg-white border border-gray-200 rounded-xl shadow-2xl p-4 md:p-6">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-2 text-lg font-normal text-gray-500"
                            x-text="isBulk ? 'Hapus ' + bulkCount + ' pertanyaan terpilih?' : 'Hapus Pertanyaan ini?'">
                        </h3>

                        {{-- Teks Pertanyaan --}}
                        <div class="mb-6 py-2 px-3 bg-gray-50 rounded-lg border border-gray-100 inline-block max-w-full">
                            <span class="font-mono text-sm font-bold text-gray-800 break-words"
