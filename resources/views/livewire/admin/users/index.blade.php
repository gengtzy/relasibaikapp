...
@close-modal.window="showModal = false">
    ...
    <div class="border border-slate-200 shadow-sm rounded-2xl bg-white">
        ...
        @if (session()->has('success'))
            <div wire:key="alert-success-{{ rand() }}" x-data="{ show: true }" x-show="show"
        ...
        {{-- Toast Notification Error (Jika hapus diri sendiri) --}}
        @if (session()->has('error'))
            <div wire:key="alert-error-{{ rand() }}" x-data="{ show: true }" x-show="show"
                x-transition:enter="transform ease-out duration-300 transition"
                x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2"
                x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0"
                x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" x-init="setTimeout(() => show = false, 4000)"
                class="fixed top-24 right-5 z-[60] flex items-center w-full max-w-sm p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-lg border-l-4 border-red-500 dark:text-gray-400 dark:bg-gray-800"
                role="alert">
                ...
                <div class="ms-3 text-sm font-normal text-slate-800">{{ session('error') }}</div>
                <button type="button" @click="show = false"
                    class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700">
                    <span class="sr-only">Close</span>
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td wire:click="editUser({{ $user->id }})" class="px-6 py-4 cursor-pointer">
                                @if ($user->role == 'admin')
                                    <span class="bg-orange-500/20 border px-2 border-orange-300 text-orange-500 p-1 font-bold text-sm rounded-lg">Admin</span>
                                @else
                                    <span class="bg-green-500/20 border px-2 border-green-300 text-green-500 p-1 font-bold text-sm rounded-lg">Masyarakat</span>
                                @endif
                            </td>
                            <td wire:click="editUser({{ $user->id }})" class="px-6 py-4 cursor-pointer">
                            ...
                            <td wire:click="editUser({{ $user->id }})" class="px-6 py-4 cursor-pointer">
                                @if ($user->superiority_role)
                                    <span class="bg-green-500/20 border px-2 border-green-300 text-green-500 p-1 font-bold text-sm rounded-2xl"><i class="fas fa-check"></i></span>
                                @elseif ($user->role == 'admin')
                                    -
                                @else
                                    <span class="bg-red-500/20 border px-2 border-red-300 text-red-500 p-1 font-bold text-sm rounded-2xl"><i class="fas fa-times"></i></span>
                                @endif
                            </td>
                            <td wire:click="editUser({{ $user->id }})" class="px-6 py-4 cursor-pointer">
                                @if ($user->superiority_role)
                                    <span class="bg-blue-500/20 border px-2 border-blue-300 text-blue-500 p-1 font-bold text-sm rounded-lg">{{ $user->superiority_role }}</span>
                                @else
                                    -
                                @endif
                            </td>
                            ...
                            <td class="flex gap-2 px-6 py-4">
                                ...
                                <button type="button"
                                    @click="
                                        ...
                                        showModal = true; 
                                        deleteId = {{ $user->id }}; 
                                        deleteCode = '{{ $user->name }}'; 
                                        $wire.set('deleteId', {{ $user->id }}, false);
        ...
        <nav class="flex pb-4 items-center flex-col md:flex-row justify-between pt-4 px-4"
            ...
            <span class="text-sm font-normal text-slate-500 mb-4 md:mb-0 block w-full md:inline md:w-auto">
                Showing
                <span class="font-semibold text-slate-700">{{ $users->firstItem() ?? 0 }}-{{ $users->lastItem() ?? 0 }}</span>
                of
                ...
                <div class="relative bg-white border border-gray-200 rounded-xl shadow-2xl p-4 md:p-6">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-2 text-lg font-normal text-gray-500">
                            {{-- Judul Dinamis --}}
                            <span x-text="isBulk ? 'Hapus ' + bulkCount + ' pengguna terpilih?' : 'Hapus Pengguna ini?'"></span>
                        </h3>
