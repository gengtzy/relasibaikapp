    ...
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                {{-- Bagian Kiri (Showing 1 to 10 of ...) --}}
                <div>
                ...
                <div>
                    <span class="relative z-0 inline-flex rtl:flex-row-reverse rounded-md shadow-sm">
                        <span>
                            ...
                            @else
                                <button type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-l-md leading-5 hover:text-blue-600 hover:bg-blue-50 focus:z-10 focus:outline-none focus:border-blue-300 focus:ring ring-blue-300 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:hover:bg-gray-700" aria-label="{{ __('pagination.previous') }}">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                    </svg>
                                </button>
                                ...
                                @foreach ($element as $page => $url)
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        @if ($page == $paginator->currentPage())
                                            {{-- TOMBOL AKTIF (DIUBAH JADI BIRU) --}}
                                            <span aria-current="page">
                                                <span class="relative z-10 inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-blue-600 bg-blue-50 border border-blue-500 cursor-default leading-5 dark:bg-blue-900 dark:border-blue-600 dark:text-blue-200">{{ $page }}</span>
                                            </span>
                                        @else
                                            {{-- TOMBOL BIASA (HOVER BIRU) --}}
